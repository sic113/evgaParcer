...
for name in nazvaniya:
    with open(f"{folder_name}/{name}", "r", encoding="UTF-8") as file:
        ...
        try:
            ...
            img_hrefs = soup.find("div", class_="simpleLens-thumbnails-container ev-vertical-thumbnails js-vertical-thumbnails").findAll("div",class_="element")
            hrefs = ""
            for i in img_hrefs:
                href = i.find("a").get("data-big-image")
                href = dom+href
                hrefs = hrefs + href + "\n"
        except:
            hrefs = "-"
        ...
        with open("evga_2.csv", "a", newline='', encoding="UTF-8") as file:
            writer = csv.writer(file)
            writer.writerow(
                (
                    art,
